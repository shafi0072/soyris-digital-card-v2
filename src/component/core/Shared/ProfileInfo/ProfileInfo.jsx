...
<<<<<<< HEAD
const ProfileInfo = ({design, infos, color }) => {
    return (
        <div className=''>
            <div  className={design==='classic' && infos?.first_name ? `pl-4 border-l-2 `: 'border-[none]'} style={{borderColor: color}}>
                <h2 className='font-bold text-xl'>
                    {infos?.prefix && infos?.prefix + '. '} {infos?.first_name} {infos?.last_name} <br />
                    {infos?.suffix} <span className='font-thin text-base '> {infos?.accreditations} </span>
                </h2>
                <h3 className='font-base text-[18px] '>
                    {infos?.job_title}
                </h3>
                <h3 className='font-semibold text-[18px] ' style={{ color: color }}>
                    {infos?.department}
                </h3>
                <h3 className='italic text-[18px] '>
                    {infos?.company}
                </h3>
            </div>
=======
